...
N_0 = 10       #Photon number offset
N_max = 3       #Changebale photon number
B_RF = 3.0E-04  #Strength of RF Field
nu_RF = 210E06  #Frequency of RF Field
...
polarization = 'plus'   #Polarization of RF Field. 'plus', 'minus', 'pi'
...
def construct_symmetrized_basis(Mk):
                ...
                for mi1 in np.arange(i1, -i1-1,-1):
                    for ms2 in np.arange(s2, -s2-1,-1):
                        for mi2 in np.arange(i2, -i2-1,-1):
                            ...
                            for Np in range(-N_max+1, N_max):
                                #for Mp in np.arange(N_max, -N_max-1,-1):
                                if polarization == 'plus':
                                    Mp = Np
                                elif polarization == 'minus':
                                    Mp = -Np
                                if ML + ms1 + mi1 + ms2 + mi2 + Mp == Mk:
                                    vec = [L, ML, ms1, mi1, ms2, mi2, Np, Mp]
    ...
    for a_ket in basic_basis:
        ...
        if ms1 == ms2 and mi1 == mi2:
            if beta == 1.0:
                vec = [alpha, beta, L, ML, ms1, mi1, ms2, mi2, Np, Mp]
                vec2 = [alpha, beta, L, ML, ms2, mi2, ms1, mi1, Np, Mp]
...
def construct_N_RF_plus_element(bra, ket):
...
def construct_H_RF_M_element(bra, ket):
    #[alpha, beta, L, ML, ms1, mi1, ms2, mi2]
    R = 0
    ...
    f_s = g_e * mu_b * B_RF / (2 * np.sqrt(N_0)) 
    f_i = g_nuc_1 * mu_b * B_RF / (2 * np.sqrt(N_0)) 
    
    if L == Lb and ML == MLb:
        
        #Term 1:
        if mi1 == mi1b and mi2 == mi2b and Np == Npb - 1 and Mp == Mpb + 1:
            if ms1 == ms1b - 1 and ms2 == ms2b:
                a_s1 = np.sqrt(s1*(s1+1) - ms1 * (ms1 + 1))
                R = R + f_s * np.sqrt(N_0 + Np + 1) * a_s1
            if ms1 == ms1b and ms2 == ms2b - 1:
                a_s2 = np.sqrt(s2*(s2+1) - ms2 * (ms2 + 1))
                R = R + f_s * np.sqrt(N_0 + Np + 1) * a_s2
                
        #Term 2:
        if ms1 == ms1b and ms2 == ms2b and Np == Npb - 1 and Mp == Mpb + 1:
            if mi1 == mi1b - 1 and mi2 == mi2b:
                a_i1 = np.sqrt(i1*(i1+1) - mi1 * (mi1 + 1))
                R = R + f_i * np.sqrt(N_0 + Np + 1) * a_i1
            if mi1 == mi1b and mi2 == mi2b - 1:
                a_i2 = np.sqrt(i2*(i2+1) - mi2 * (mi2 + 1))
                R = R + f_i * np.sqrt(N_0 + Np + 1) * a_i2
            
        #Term 3:
        if mi1 == mi1b and mi2 == mi2b and Np == Npb + 1 and Mp == Mpb - 1:
            if ms1 == ms1b + 1 and ms2 == ms2b:
                b_s1 = np.sqrt(s1*(s1+1) - ms1 * (ms1 - 1))
                R = R + f_s * np.sqrt(N_0 + Np) * b_s1
            if ms1 == ms1b and ms2 == ms2b + 1:
                b_s2 = np.sqrt(s2*(s2+1) - ms2 * (ms2 - 1))
                R = R + f_s * np.sqrt(N_0 + Np) * b_s2
                
        #Term 4:
        if ms1 == ms1b and ms2 == ms2b and Np == Npb + 1 and Mp == Mpb - 1:
            if mi1 == mi1b + 1 and mi2 == mi2b:
                b_i1 = np.sqrt(i1*(i1+1) - mi1 * (mi1 - 1))
                R = R + f_i * np.sqrt(N_0 + Np) * b_i1
            if mi1 == mi1b and mi2 == mi2b + 1:
                b_i2 = np.sqrt(i2*(i2+1) - mi2 * (mi2 - 1))
                R = R + f_i * np.sqrt(N_0 + Np) * b_i2
                
        #Term 5:
        if ms1 == ms1b and ms2 == ms2b and mi1 == mi1b and mi2 == mi2b and Np == Npb and Mp == Mpb:
            R = R + h_planck * nu_RF * Np
            
            
    return R

def construct_H_RF_P_element(bra, ket):
    #[alpha, beta, L, ML, ms1, mi1, ms2, mi2]
    R = 0
    ...
    if L == Lb and ML == MLb:
        ...
        if mi1 == mi1b and mi2 == mi2b and Np == Npb + 1 and Mp == Mpb + 1:
            if ms1 == ms1b - 1 and ms2 == ms2b:
                a_s1 = np.sqrt(s1*(s1+1) - ms1 * (ms1 + 1))
                R = R + f_s * np.sqrt(N_0 + Np) * a_s1
            if ms1 == ms1b and ms2 == ms2b - 1:
                a_s2 = np.sqrt(s2*(s2+1) - ms2 * (ms2 + 1))
                R = R + f_s * np.sqrt(N_0 + Np) * a_s2
                
        #Term 2:
        if ms1 == ms1b and ms2 == ms2b and Np == Npb + 1 and Mp == Mpb + 1:
            if mi1 == mi1b - 1 and mi2 == mi2b:
                a_i1 = np.sqrt(i1*(i1+1) - mi1 * (mi1 + 1))
                R = R + f_i * np.sqrt(N_0 + Np) * a_i1
            if mi1 == mi1b and mi2 == mi2b - 1:
                a_i2 = np.sqrt(i2*(i2+1) - mi2 * (mi2 + 1))
                R = R + f_i * np.sqrt(N_0 + Np) * a_i2
            
        #Term 3:
        if mi1 == mi1b and mi2 == mi2b and Np == Npb - 1 and Mp == Mpb - 1:
            if ms1 == ms1b + 1 and ms2 == ms2b:
                b_s1 = np.sqrt(s1*(s1+1) - ms1 * (ms1 - 1))
                R = R + f_s * np.sqrt(N_0 + Np + 1) * b_s1
            if ms1 == ms1b and ms2 == ms2b + 1:
                b_s2 = np.sqrt(s2*(s2+1) - ms2 * (ms2 - 1))
                R = R + f_s * np.sqrt(N_0 + Np + 1) * b_s2
                
        #Term 4:
        if ms1 == ms1b and ms2 == ms2b and Np == Npb - 1 and Mp == Mpb - 1:
            if mi1 == mi1b + 1 and mi2 == mi2b:
                b_i1 = np.sqrt(i1*(i1+1) - mi1 * (mi1 - 1))
                R = R + f_i * np.sqrt(N_0 + Np + 1) * b_i1
            if mi1 == mi1b and mi2 == mi2b + 1:
                b_i2 = np.sqrt(i2*(i2+1) - mi2 * (mi2 - 1))
                R = R + f_i * np.sqrt(N_0 + Np + 1) * b_i2
...
def construct_operator(basis, constructor_function):
    ...
    for i in range(len(basis)):
        for j in range(len(basis)):
            ...
            bra2 = [L, ML, ms1, mi1, ms2, mi2, Np, Mp]
            ket2 = [Lb, MLb, ms1b, mi1b, ms2b, mi2b, Npb, Mpb]
            
            O[i,j] = alpha_i * alpha_j * constructor_function(bra2, ket2)
            
            ket2 = [Lb, MLb, ms2b, mi2b, ms1b, mi1b, Npb, Mpb]
            O[i,j] = O[i,j] + alpha_i * alpha_j * beta_j * constructor_function(bra2, ket2)
    
            bra2 = [L, ML, ms2, mi2, ms1, mi1, Np, Mp]
            ket2 = [Lb, MLb, ms1b, mi1b, ms2b, mi2b, Npb, Mpb]
...
def generate_ham(B_field, M_tot):
    ...
    N_RF_P = construct_operator(sym_bas, construct_N_RF_plus_element)
    
    H_RF = construct_operator(sym_bas, construct_H_RF_P_element)
    
    #H_RF = np.full((basis_size, basis_size), 0.0)
    ...
    H_RF = 1.0/conf.E_scaling * H_RF
    
    H_infty = H_hyp + H_zee + H_RF
    ...
    H_RF_t = np.linalg.multi_dot([np.linalg.inv(Ut), H_RF, Ut])
    N_RF_P_t = np.linalg.multi_dot([np.linalg.inv(Ut), N_RF_P, Ut])
    ...
    f = tb.open_file(conf.working_dir + '\\in\\H_RF.h5', 'w')
    f.create_array('/', 'ARR', H_RF_t)
    f.close()
